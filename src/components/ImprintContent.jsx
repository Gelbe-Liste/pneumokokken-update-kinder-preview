...
export default function ImprintContent({ page, imprint }) {
  return <ContentCard wide><p className="page-kicker">{page.kicker}</p><h2>{page.title}</h2><div className="imprint-grid">
    ...
    {imprint.sponsoring && <section className="imprint-section imprint-section--full"><h3>{imprint.sponsoring.heading}</h3><p>{imprint.sponsoring.text}</p>{imprint.sponsoring.note && <p><em>{imprint.sponsoring.note}</em></p>}</section>}
